...
print(f"After comparison line: {len(c)}")

# 3. Remove radio inputs, tabs, and Ruta Original section
...
print(f"After removing content-f: {len(c)}")
...
print(f"After removing content-r: {len(c)}")

# 5. Remove route-wrap wrapper
c = c.replace('<div class="route-wrap">\n        ', '')
c = c.replace('\n        </div><!-- /route-wrap -->', '')
print(f"After route-wrap: {len(c)}")

# 6. Remove route-content wrappers
c = c.replace('<div class="route-content" id="content-r2">\n\n                ', '\n                ')
c = c.replace('\n        </div><!-- /content-r2 -->', '')
print(f"After route-content: {len(c)}")
...
c = c.replace(old_css, '')
print(f"After CSS: {len(c)}")

# 8. Milan collapsed in Ruta Real
...
print(f"After moving budget: {len(c)}")

script_start = c.find('<script>')
c = c[:script_start] + '\n\n' + pw_section + '\n\n' + budget_section + '\n\n' + c[script_start:]
print(f"After inserting: {len(c)}")
...
if fwd > 0 and real > fwd:
    c = c[:fwd] + c[real:]
print(f"After clean budget: {len(c)}")
...
bullet = c.find('Ginebra ')
print(f"Ginebra Neuch: {'Ginebra ' in c and 'Neuch' in c}")
print(f"Ruta Original text: {'Ruta Original' in c}")
print(f"Ruta Alternativa text: {'Ruta Alternativa' in c}")
print(f"PW bottom: {c.find('passwordGate') > len(c) * 0.6}")
print(f"Budget bottom: {c.find('PRESUPUESTO') > len(c) * 0.6}")
print(f"Hero below: {c.find('hero-cover') > c.find('13')}")
print(f"PW count: {c.count('passwordGate')}")